...
def h(state: State, final_pos: tuple) -> int:
    ...
    h=0
    if (state.n_pages !=0):
        h=dist_man(state.player_pos,state.pages_pos[0])
        for i in range (state.n_pages):
            new_dist=dist_man(state.player_pos,state.pages_pos[i])
            if (new_dist<h):
                h = new_dist
    else:
...
class PageCollect(Problem):
    ...
    def result(self, state: State, action: str) -> State:
        ...
        if old_y == self.goal_pos[1] and old_x == self.goal_pos[0]:
            grid[old_y][old_x] = "X"
        else:
            grid[old_y][old_x] = " "
        grid[new_y][new_x] = "@"
    ...
    def h(self, node: Node) -> int:
        ...
        if (node.state.n_pages !=0):
            h=dist_man(node.state.player_pos,node.state.pages_pos[0])
            for i in range (node.state.n_pages):
                new_dist=dist_man(node.state.player_pos,node.state.pages_pos[i])
                if (new_dist<h):
                    h = new_dist
        else:
